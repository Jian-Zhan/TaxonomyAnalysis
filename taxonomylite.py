...
tid = taxa_db.name_to_tid("Actinomyces sp. oral taxon 181")
tids1 = taxa_db.name_regex_to_tids("Panthera l.*")

immediate_children = taxa_db.children(tid)
all_children = taxa_db.children(tid, deep=True)

print taxa_db.tid_to_rank(tid)
print taxa_db.tid_to_name(tid)


To create a new database from NCBI sources:
from taxonomylite import Taxonomy
taxa_db = Taxonomy.from_source("taxonomy.db")   ## This process may take some time.
'''
...
class Taxonomy(object):
    ...
    def __init__(self, store_path):
        ...
        def regexp_fn(expr, item):
            reg = re.compile(expr)
            return reg.search(item) is not None
        self.connection.create_function("REGEXP",  2, regexp_fn)
    ...
    def name_regex_to_tids(self, regex):
        """Find out taxonomic id numbers of which the names match the regex

        Parameters
        ----------
        regex: str
            Regular expression like "A.*baumannii"
        ...
        """
        result = self.execute(
            "SELECT taxa_id FROM taxonomy WHERE taxa_name REGEXP ?", (regex,)).fetchall()
        if result:
